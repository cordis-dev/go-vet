		...
		for lineno, line := range lines {
			...
			if m := asmTEXT.FindStringSubmatch(line); m != nil {
				...
				if arch == "" {
					...
					if arch == "" {
						f.WarnfV2("asmdecl", token.NoPos, "%s: cannot determine architecture for assembly file", f.name)
						continue Files
					}
				}
				fnName = m[2]
				if pkgName := strings.TrimSpace(m[1]); pkgName != "" {
					...
					if pkgName != f.pkg.path {
						f.WarnfV2("asmdecl", token.NoPos, "%s:%d: [%s] cannot check cross-package assembly function: %s is in package %s", f.name, lineno, arch, fnName, pkgName)
						fn = nil
						fnName = ""
						continue
					}
